...
- `api/oauth.ts` + `cache/token-store.ts` — deploy·ncs·logncrash 검색 공용. UAK → access_token 교환 후 단기 캐시 (계정 단위 토큰이라 profile 캐시 공유, [[adr-007]], [[adr-020]], [[adr-024]]). 캐시에 자격 지문을 저장해 자격 변경 시 무효화 ([[adr-021]])
