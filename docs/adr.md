...
## ADR-013: IaaS 멀티 서비스 endpoint 해석 — image·network catalog host 맵 추가 (정적 맵 유지)
...
- **결정**: image(Glance v2)·network(NHN VPC) endpoint 도 compute 와 동일하게 region 별 **정적 host 맵**으로 해석한다.
  ...
  - image 는 `<region>-api-image-infrastructure...`, network 는 `<region>-api-network-infrastructure...` (둘 다 실측 확정)로 compute 와 다른 host 지만 **같은 Keystone 토큰**(`X-Auth-Token`)을 재사용한다.
  - Glance 경로는 tenant segment 가 없다(`/v2/images`, 실측 확정). NHN VPC 경로도 tenant segment 가 없다(`/v2.0/vpcs`·`/v2.0/vpcsubnets`, serviceCatalog 실측 확정). NHN VPC 는 raw Neutron `/v2.0/networks` 가 아니라 NHN 고유 경로다.
...
- **맥락**: instance images(image type)·network list(network type)는 service catalog type 이 compute 가 아닌 명령이다.
  - 기존 코드는 compute host 만 정적 맵으로 갖고 serviceCatalog 를 파싱하지 않는다([[adr-005]]).
  - region 별 image·network host 도 compute 와 같은 정적 패턴이라 맵을 더해 해결된다 — image 가 첫 사례, network 가 두 번째 사례로 같은 패턴이 재사용됨을 확인.
- **대안 기각**:
...
- **트레이드오프**:
  - region 코드가 compute·image·network **세 host 맵**에 중복된다(image 때 둘 → network 추가로 셋) — region 추가 시 동기화 누락 위험.
    구현 시 세 맵 key 집합 일치를 성공 기준 grep 으로 확인한다 (상시 런타임 가드는 아님 — 추가 시 후속 task). 서비스 type 이 더 늘어 맵 관리가 부담이 되는 시점에 동적 catalog 파싱 재검토 — 그 임계 사례가 하나 더 쌓였다.
  - host 패턴·tenant 유무를 docs 만으로 확정하지 못해 실측으로 확정했다(추측 구현 금지).
  - **kr1/kr2 만 publicURL 실측 확정. kr3/jp1 IMAGE_HOST·NETWORK_HOST 는 같은 패턴으로 추론**(미실측) — 자격증명 확보 시 후속 실측. 첫 호출이 host 에서 실패하면 `getaddrinfo ENOTFOUND` 로만 드러난다.
