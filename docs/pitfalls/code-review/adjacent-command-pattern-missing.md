...
**Good**: 같은 도메인 (`src/commands/<svc>/`) 신규 명령 작성 시 인접 파일 (`list.ts`, `get.ts`, `create.ts` 등) 의 enrich, cleanup, 출력 분기 패턴을 grep 으로 먼저 확인하고 그대로 적용. 일관성이 회귀 방어선.
**검출**: phase 작성 / review 시 `grep -nE "try\s*\{|catch\s*\(|new Map" src/commands/<svc>/*.ts` 결과를 신규 명령과 인접 명령 사이 diff. 인접 명령에 있는 가드가 신규 명령에 없으면 의도적인지 확인.
