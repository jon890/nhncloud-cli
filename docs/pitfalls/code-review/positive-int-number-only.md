...
**Good**: `POSITIVE_INTEGER_PATTERN = /^[1-9]\d*$/` 정규식으로 표기 자체를 사전 검증한 뒤 `Number()`. 빈 문자열·소수·지수·공백을 일관 거부. 에러 메시지는 `JSON.stringify(value)`로 입력을 표기해 빈 괄호 방지. 0을 허용해야 하면 `NON_NEGATIVE_INTEGER_PATTERN = /^(0|[1-9]\d*)$/`를 쓴다. 두 상수는 `src/commands/parse-options.ts`에 있고 `parseIntegerOption`이 range로 골라 쓴다.
