...
- 초와 시간대를 갖춘 ISO8601 직접 입력은 그대로 통과한다 (예: `2026-08-03T00:00:00+09:00`, `2026-08-03T00:00:00Z`).
  날짜만 주거나 초·시간대가 빠진 값은 서버가 `invalid datetime format` 400 으로 거부하므로 CLI 가 먼저 막는다([[adr-030]]).
...
| `--file <path>` | 조건 | 본문을 읽을 파일 경로 (stat 가드와 8MB 한도) |
