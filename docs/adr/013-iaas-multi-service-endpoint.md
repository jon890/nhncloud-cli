...
- **결정**: image(Glance v2)·network(NHN VPC) endpoint 도 compute 와 동일하게 region 별 **정적 host 맵**으로 해석한다.
  ...
  - image 는 `<region>-api-image-infrastructure...`, network 는 `<region>-api-network-infrastructure...`, blockstorage 는 `<region>-api-block-storage-infrastructure...`(catalog type `volumev2`)로 compute와 host가 다르지만 **같은 Keystone 토큰**(`X-Auth-Token`)을 재사용한다.
