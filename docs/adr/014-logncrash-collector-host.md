...
- **결정**: 로그 전송(`logncrash send`)은 검색과 다른 collector host 와 인증 모델을 쓴다.
  ...
  - 인증: 헤더 인증 없음 — body 의 `projectName` 필드에 appkey 를 넣어 프로젝트를 식별한다 (검색의 UAK OAuth Bearer와 다른 모델, UAK 토큰·서비스 secret 불요)
...
- **맥락**: Log & Crash 는 검색(read)과 수집(write)의 host·인증이 서로 다르다.
  - Search v3는 UAK OAuth Bearer 인증([[adr-024]]), 수집은 appkey 만으로 식별한다.
  - 두 동작을 같은 host·인증으로 가정하면 전송이 401 또는 404 로 실패한다.
- **대안 기각**:
  - 검색 host 재사용 — 수집 엔드포인트가 없어 404.
  - 검색용 `X-NHN-Authorization` 헤더 전송 — 수집은 헤더 인증을 받지 않으며 UAK 토큰을 요구하지 않는다.
