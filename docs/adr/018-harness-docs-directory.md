...
- **결정**: 항목이 계속 append 되는 누적 docs 를 단일 파일이 아니라 **항목 1개 = 파일 1개 + INDEX 라우터**로 운영한다.
  ...
  - 회고 절차: build-with-teams 9-7·planning 축적 규칙·review-fix 6.5 에 분산되던 회고 절차를 `_shared/retros/{critic,code-reviewer,docs-verifier}-retro.md` 역할별 단일 소스로 모은다(거울 구조 — retro 는 *절차*만 담고, 데이터 갱신 위치는 해당 pitfalls 카테고리·planning 영향 표가 단일 소스).
...
- **대안 기각**:
  ...
  - 회고 데이터까지 retro 에 로그형으로 누적 — 별도 데이터 문서가 새 rot 소스가 되어 거울 구조를 깬다(데이터는 pitfalls·영향 표가 단일 소스로 유지하고 retro 는 절차만 담는다).
