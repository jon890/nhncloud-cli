...
- **맥락**: 캐시가 `profile`(+region) 만으로 keying 되어 `configure` 로 자격을 교체해도 이전 자격으로 발급된 토큰이 만료(약 24h) 전까지 재사용됐다.
  ...
  - deploy·ncs·logncrash 검색은 같은 계정 단위 User Access Key OAuth 토큰을 공유한다
    ([[adr-007]], [[adr-020]], [[adr-024]]).
    따라서 파일명 `deploy-token`은 실제 소유 범위를 잘못 표시했다.
- **대안 기각**:
...
- **적용 범위**: OAuth 캐시(deploy·ncs·logncrash 검색 공용)와 iaas Keystone 캐시 양쪽에 적용한다.
  지문 필드가 없거나 옛 파일명인 구 캐시는 type 가드 실패로 null이 되어 자연 재발급된다.
  하위 호환 동작은 [[adr-007]], [[adr-010]]의 기존 재발급 복구와 같은 패턴이다.
