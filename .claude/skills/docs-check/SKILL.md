...
대상: `docs/*.md` / `CLAUDE.md` / `README.md` / `skills/nhncloud-cli/SKILL.md` / `tasks/**/*.md` / `.claude/skills/**/*.md`.
...
docs-check 의 6축 검증은 **반드시** custom agent `nhncloud-cli-docs-verifier` (`.claude/agents/nhncloud-cli-docs-verifier.md`) 에 위임한다. agent 본문이 검증 항목·자동 grep 명령·도메인 지식의 단일 소스 — main session 이 직접 6축 grep 을 따라 적는 순간 정의 두 곳 동기화 부담 발생 (거울 구조 원칙 위반).
