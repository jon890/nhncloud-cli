...
**Why**: PR #16 (plan012) critic MAJOR — phase-01 이 ADR-014 + CLAUDE.md + flow + code-architecture 편집을 묶어 pitfall [[new-command-docs-required-skip]]·갱신 시점 분리와 충돌. team-lead docs-first 분리로 해소. plan012~019 처럼 일괄 생성된 backlog task 군은 모두 같은 구조라 매 plan 재발 가능 — 실행 전 선제 분리.
