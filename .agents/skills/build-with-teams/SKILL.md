...
**사전 해소 점검 (필수)**: code-reviewer 검사 시작 전에 `.Codex/skills/_shared/code-review-pitfalls.md` 의 모든 항목이 코드에 적용됐는지 확인.
...
7. **review 회고 (조건부 필수 — 학습 루프)** — PR 생성 직후, 팀 shutdown 직전
   ...
   - **critic** REVISE 지적 중 *반복 가능성* 있는 패턴 → `.Codex/skills/_shared/common-pitfalls.md` 해당 섹션에 항목 추가
   - **code-reviewer** FIX_NEEDED 지적 중 *반복 가능성* 있는 패턴 → `.Codex/skills/_shared/code-review-pitfalls.md` 해당 카테고리에 항목 추가 (또는 새 카테고리 신설)
