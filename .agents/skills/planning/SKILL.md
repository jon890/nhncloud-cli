...
새 기능/변경사항을 구현하기 전 8단계 설계 워크플로우. 모호한 부분을 모두 해소하고, 문서를 정비한 뒤 `/build-with-teams`로 실행에 넘긴다.
...
**docs-verifier 반복 지적은 별도 회고 문서로 분리하지 않는다** — 아래 "문서 단일 출처 원칙" 섹션 참조. docs-verifier 의 반복 지적은 본 SKILL 8단계 A항 docs 영향 표에 행을 추가하거나 기존 행을 보강하는 방식으로 반영한다.

## 문서 단일 출처 원칙

build-with-teams 의 docs-verifier 검증 항목과 본 SKILL 의 docs 영향 표가 **같은 정의를 두 곳에 따로 두지 않는다** 는 원칙. 같은 점검 목록을 두 곳에 유지하면 시간이 지나며 한쪽만 갱신되는 사고가 발생한다.

**규칙**:

1. **단일 출처**: 본 SKILL 8단계 A항 "변경 유형별 docs 영향 표" 가 docs 갱신 기준의 **유일한 정의**이다. 모든 변경 유형 (새 resolver, 새 명령, ADR 추가 등) 은 이 표에 행으로 등록되어 있어야 한다.

2. **참조만 허용**: `build-with-teams/SKILL.md` 의 docs-verifier 검증 항목 7~10 (planning docs 영향 표 100% 적용 / 역참조 / 갱신 시점 분리 / 공개 스킬 dogfooding) 은 위 표를 참조만 한다. 별도 점검 목록을 보유하지 않는다. docs-verifier 가 *"별도 체크 항목 X 도 보겠다"* 며 자체 항목을 늘리면 기준이 둘로 갈라진다.

3. **별도 회고 *데이터* docs 신설 금지** — 절차 단일 출처는 `_shared/retros/docs-verifier-retro.md`, 데이터 단일 출처는 본 8단계 A항 표이다. retro 는 절차만 담고 데이터(표 행)는 여기 반영한다.
...
5. **새 변경 유형 추가**: 새 외부 통합 모듈 / 새 CLI 도메인 영역 등이 등장하면 본 표에 행 1줄 추가한다. 그러면 docs-verifier 가 그 변경 유형을 검증하기 시작한다 (별도 코드 변경 불요).

**이유**: 이전엔 docs-verifier 가 같은 항목 (`code-architecture.md` 디렉터리 트리, README 사용법, "N개 명령" 카운트) 을 반복 지적했고, 그때마다 별도 docs-sync-checklist 를 만들 유인이 있었다. 기준을 두 곳에 두면 한쪽만 갱신되어 다른 쪽이 낡는다. 기준을 한 곳에 두면 누락 가능성을 줄일 수 있다.
...
- **가독성 + 토큰 효율**: `AGENTS.md` "docs / ADR 작성 형식" 6가지 패턴 따른다 (패턴 정의는 거기에 단일 소스)
...
for f in <변경 파일>; do
  ...
  grep -nE "①|②|③|④|⑤|⑥|⑦|⑧|⑨" "$f"
  # 한국어 표현 정책 — 상세 치환표는 전역 ~/.claude/rules/korean-style.md 참조
  # 전역 치환표의 단어가 남아 있으면 문맥에 맞게 한국어로 풀어 쓴다.
done
```
...
- 외래어 치환 — 전역 `~/.claude/rules/korean-style.md` 의 권장 표현을 적용
...
문서 단일 출처 원칙 — 패턴 정의는 `AGENTS.md`와 전역 한국어 표현 정책을 단일 소스로 둔다. 본 섹션은 *검증 시점 + 실행 명령*만 명시한다.

## 완료 후

8단계가 끝나면 사용자에게 안내:

> 설계가 완료되었습니다. `/build-with-teams`로 구현을 시작할까요?
