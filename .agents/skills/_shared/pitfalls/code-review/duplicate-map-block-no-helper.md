...
**증상**: 같은 입력 (`ids: string[]`, `targets: string[]`) 을 같은 외부 호출 시퀀스 (`trim → validate → client.getXxx → 변환`) 로 변환하는 15 줄 블록이 4 명령 파일에 그대로 복사됨.
  critic 이 sub-pattern 으로 지적했으나 "executor 재량" 으로 흡수 안 함 → code-reviewer 가 다시 지적 → 사후 별도 PR review-fix 로 추출.
**Good**: phase 작성 시 "동일 변환 N 파일 복붙" 이 보이면 plan 본문에 helper 명시 (`src/services/<svc>/` 또는 `src/commands/<svc>/helpers.ts` 신규). critic 의 sub-pattern 도 MAJOR 로 승격할지 판단 — 4 파일 이상 복붙은 사후 review-fix 보다 phase 안에서 추출하는 게 cheaper.
**검출**: `git diff --name-only` 결과의 같은 `src/commands/<svc>/` 파일이 3 개 이상이고 각 diff 의 +라인 패턴이 `for/map(... await ...)` 형태로 유사하면 추출 후보.
**Why**: phase plan 이 동일 async map/for 변환을 여러 명령 파일에 명시하면 적용 후 review 에서 helper 추출 요구가 반복된다.
