...
스테이지 설명 변경과 복원, `--dry-run` 조회로 확인했다.
실제 쓰기는 설명 변경·복원 두 번뿐이고 원래 값으로 되돌렸다.

- **수정 응답에 두 필드가 있었다.** 문서 예시가 응답 전체를 담지 않았다.
  코드는 고치지 않는다 — `isUpdatedStage` 가 출력에 쓰는 다섯 필드만 요구해
  두 필드가 있든 없든 통과한다. ADR-028 의 근거 서술만 정정했다.
- **`backendEndpointUrl` 이 기존 값 그대로 유지됐다.** 설명만 바꾸는 호출이 URL 을 지우지 않는다.
  `stageDescription` 미전송 동작은 확인 대상이 아니게 됐다 —
  명령이 미지정 필드를 항상 기존 값으로 채우므로 CLI 로는 그 상태를 만들 수 없다.
- **`--dry-run` 범위가 실제와 맞았다.** 루트 경로에 `applyChildPath` 를 걸면 68건이 나오고
  `resource list` 전체 건수와 같다. `applyChildPath` 가 없으면 1건이며 추정 경고도 나오지 않는다.
- CORS 경고와 타입 거부, `--yes` 누락 거부가 모두 동작했다.
  `--quiet` 은 stdout 에 식별자만 내고 경고는 stderr 로 분리됐다.
