...
- [ ] `src/utils/time.ts`
  ...
  - 출력은 타임존 오프셋 포함 ISO8601 (`YYYY-MM-DDThh:mm:ss±hh:mm`). `Date.toISOString()` 은 `Z`(UTC) 만 내므로 로컬 오프셋을 수동 조립
  - `export function assertSearchRange(fromIso: string, toIso: string): void` — flow.md 시간 제약 사전 검증
    - `from > to` 면 `throw new NhnCloudCliError("from 이 to 보다 늦습니다", EXIT_PARAM_ERROR)`
    - 범위 `(to - from)` 가 31일 초과면 `throw new NhnCloudCliError("검색 범위는 31일 이하여야 합니다", EXIT_PARAM_ERROR)`
    - `from` 이 현재 기준 90일 이전이면 `throw new NhnCloudCliError("검색 시작은 최근 90일 이내여야 합니다", EXIT_PARAM_ERROR)`
...
grep -c "export function assertSearchRange" src/utils/time.ts  # 기대: 1
# 오프셋 형식 — Z 단독이 아닌 ±hh:mm 포함 (수동 조립 흔적)
grep -cE "getTimezoneOffset|[+-]\\\$\{|padStart" src/utils/time.ts   # 기대: >=1
```
