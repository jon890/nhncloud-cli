...
| `nks cluster resize <cluster>` | `POST /clusters/{cluster}/actions/resize` | `{ nodegroup, node_count, nodes_to_remove? }` | `{ uuid: string }` |
| `nks cluster set-ipacl <cluster>` | `POST /clusters/{cluster}/api_ep_ipacl` | JSON file raw payload | `{ uuid: string }` |
