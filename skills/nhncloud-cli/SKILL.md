---
name: nhncloud-cli
description: NHN Cloud 서비스 CLI. 자격증명 설정(configure), Log & Crash 로그 검색(logncrash search), Deploy 배포 실행(deploy), Compute 인스턴스 관리(instance — 목록·발급·전원 제어) 등 NHN Cloud PaaS API 를 터미널·AI 에이전트에서 호출한다.
---
