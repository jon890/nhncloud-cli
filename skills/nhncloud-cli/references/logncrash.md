...
**검색 기간을 좁히는 것은 해법이 아니다.**
호출 한 번의 비용이 조회 기간에 거의 비례하지 않기 때문이다.
같은 구간을 여러 창으로 나누면 오히려 제한을 더 빨리 소모한다.
시간을 두고 다시 실행해야 한다.

CLI는 자동으로 다시 시도하지 않는다.
서버가 남은 시간을 알려주지 않아 얼마나 기다릴지 알 수 없고, 회복이 느려 대기가 실효를 갖기 어렵다.
...
**비용을 정하는 것은 호출 횟수와 쿼리의 선택성이다.** 조회 기간이 아니다.
기간을 6배로 늘려도 한 호출의 비용은 거의 그대로였다.
반면 `*` 대신 필드로 좁힌 쿼리는 같은 조회를 10분의 1 이하로 끝냈다.

**그래서 쿼리를 먼저 좁힌다.** 기간을 좁히는 것보다 효과가 크다.
`logSource`·`logLevel`·`host` 처럼 인덱스가 있는 필드로 걸러내면 호출당 비용이 급감한다.
`*` 로 넓게 받아 로컬에서 거르는 방식은 제한을 가장 빨리 소모한다.
...
**재시도할 때 오류를 구분한다.**
조회 횟수 제한과 서버 500은 기다리면 풀리지만, 입력 오류와 설정 오류는 풀리지 않는다.
구분하지 않고 백오프를 걸면 풀리지 않는 오류에 대기 시간만 쓴다.

| 종료 코드 | 의미 | 기다리면 풀리나 |
|---|---|---|
| 1 | API 오류, 봉투 실패 (조회 횟수 제한 포함) | 그렇다 |
| 2 | 인증 실패 | 아니다 |
| 3 | 입력 오류 | 아니다 |
| 4 | 설정 오류 | 아니다 |

**백오프는 분 단위로 짧게 잡지 않는다.**
회복이 느려서, 넓은 쿼리로 소모한 뒤에는 몇십 초나 몇 분을 기다려도 한 번 호출할 몫이 돌아오지 않는다.
좁은 쿼리라면 훨씬 빨리 회복되지만, 정확한 값은 프로젝트와 쿼리마다 달라 넉넉히 잡는 편이 안전하다.
