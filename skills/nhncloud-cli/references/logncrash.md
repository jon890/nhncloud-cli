...
**비용은 호출 횟수가 지배한다.** 조회 기간이나 받은 건수가 아니다.
호출 하나의 고정 비용이 크고 회복이 느려서, 같은 데이터를 몇 번에 나눠 받았는지가 성패를 가른다.
...
회복 속도에 비해 호출 하나의 비용이 커서, 몇십 초나 몇 분을 기다려도 한 번 호출할 몫이 돌아오지 않는다.
정확한 값은 프로젝트마다 다르므로 짧은 간격으로 두 번 시도해 보고 정하지 말고, 넉넉히 잡는다.
